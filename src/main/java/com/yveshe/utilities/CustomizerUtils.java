...
public class CustomizerUtils {
     ...
     * 设置主题<br>
     *
     * 说明: ChartFactory生成JFreeChart对象之前进行设置生效
     *
     * @param theme
     */
     ...
     * 一般常用设置
     */
    public static void setGeneral(JFreeChart chart) {
     ...
     * @param chart
     */
    private static void setBackColor(JFreeChart chart) {// 0.0~1.0之间,数值越小越透明
        ...
        // 透明度 (绘制的图表线条的透明度)
        plot.setForegroundAlpha(1.0f); // 前景背景颜色默认为灰色
     ...
     *            启用
     */
    private static void setAntiAlias(JFreeChart chart, boolean flag) {
     ...
     * @param flag
     */
    private static void setBorderVisible(JFreeChart chart, boolean flag) {
        chart.setBorderVisible(flag);
    }
     ...
     * 不同的绘图器,设置不同的效果<br>
     *
     * 关于Plot分类: https://blog.csdn.net/u011479200/article/details/81191239
     *
     * @param chart
     */
    public static void setPlot(JFreeChart chart) {
        Plot plot = chart.getPlot();
        if (plot instanceof PiePlot) {// 包括PiePlot3D 和 RingPlot
            PiePlot piePlot = (PiePlot) plot;
            setPiePlot(piePlot);

        } else if (plot instanceof CategoryPlot) {
            CategoryPlot cPlot = (CategoryPlot) plot;
            setCategoryPlot(cPlot);
     ...
     * @param xyPlot
     */
    private static void setXYPlot(XYPlot xyPlot) {

        // 默认数据拐点为不同类型的图形,这里设置成统一的小圆点
        XYItemRenderer renderer = xyPlot.getRenderer();
        XYDataset dataset = xyPlot.getDataset();
        if (dataset != null) {
            for (int i = 0; i < dataset.getSeriesCount(); i++) {
                renderer.setSeriesShape(i, new java.awt.geom.Ellipse2D.Double(-2D, -2D, 4D, 4D));
            }
        }

        // 设置时间序列图样式
        setTimeSeriesRender(xyPlot, false, false);
     ...
     *            是否显示数据节点形状
     */
    private static void setTimeSeriesRender(XYPlot plot, boolean isShowData, boolean isShapesVisible) {
    ...
    private static void setTimeSeriesBarRender(XYPlot xyplot, boolean isShowDataLabels) {
     ...
     * @param axis
     */
    private static void setXY_XAixs(XYPlot plot) {
     ...
     * @param axis
     */
    private static void setXY_YAixs(XYPlot plot) {
        Color lineColor = new Color(31, 121, 170);
    ...
    private static void setPiePlot(PiePlot plot) {
        ...
        plot.setBaseSectionOutlinePaint(Color.WHITE); // 注意:
        // setBaseSectionOutlineStroke和setBaseSectionOutlinePaint设置也会影响图例的显示
     ...
     * 设置饼状图渲染
     */
    private static void setPieRender(PiePlot piePlot) {

        piePlot.setNoDataMessage(NO_DATA_MSG);
        piePlot.setInsets(new RectangleInsets(10, 10, 5, 10));
    ...
    private static void setCategoryPlot(CategoryPlot plot) {
        ...
        // 由于柱状图的轮廓线设置不能像和饼状图一样,这里要用到Renderer
        CategoryItemRenderer renderer = plot.getRenderer();
        CategoryDataset dataset = plot.getDataset();

        if (renderer instanceof BarRenderer) {// 1.柱状图
            ((BarRenderer) renderer).setDrawBarOutline(true);// 默认为不开启
            if (dataset != null) {
                for (int i = 0; i < dataset.getRowKeys().size(); i++) {
                    renderer.setSeriesOutlinePaint(i, Color.BLACK); // 默认为灰色
                    renderer.setSeriesOutlineStroke(i, new BasicStroke(1.0f)); // 柱状图边框线条粗细
        ...
        } else if (renderer instanceof LineAndShapeRenderer) {// 2.折线图(普通线图,不包括XY线图)
            LineAndShapeRenderer lineRenderer = (LineAndShapeRenderer) renderer;

            if (dataset != null) {
                for (int i = 0; i < dataset.getRowKeys().size(); i++) {// 拐点样式和大小设置
                    lineRenderer.setSeriesShapesVisible(i, true);
                    lineRenderer.setSeriesShape(i, new java.awt.geom.Ellipse2D.Double(-2D, -2D, 4D, 4D));
                }
            }
        ...
        } else if (renderer instanceof StackedBarRenderer) {

            // 设置堆积柱状图渲染
            setStackBarRender(plot);
        }
    ...
    private static void setStackBarRender(CategoryPlot plot) {
        plot.setNoDataMessage(NO_DATA_MSG);
        plot.setInsets(new RectangleInsets(10, 10, 5, 10));
